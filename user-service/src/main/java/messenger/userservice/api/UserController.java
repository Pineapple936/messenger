...
public class UserController {
    ...
    public ResponseEntity<UserDetailsResponse> create(@Valid @RequestBody CreateUserDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(userDetailsMapper.toResponse(userDetailsService.save(dto)));
    }

    @GetMapping("/{userId}")
    public ResponseEntity<UserDetailsResponse> getByUserId(@PathVariable Long userId) {
        return ResponseEntity.ok(userDetailsMapper.toResponse(userDetailsService.findByUserId(userId)));
    }
    ...
    public ResponseEntity<List<UserDetailsResponse>> searchUsersByTag(@RequestParam String tag) {
        return ResponseEntity.ok(userDetailsService.searchUsersByTag(tag)
                .stream()
                .map(userDetailsMapper::toResponse)
                .toList()
        );
    }
    ...
    public ResponseEntity<UserDetailsResponse> edit(@RequestHeader(USER_ID_HEADER) Long userId,
        ...
        return ResponseEntity.ok(userDetailsMapper.toResponse(userDetailsService.editUser(userId, dto)));
    }

    @PutMapping("/edit/avatar")
    public ResponseEntity<UserDetailsResponse> editAvatar(@RequestHeader(USER_ID_HEADER) Long userId,
                                                          @RequestBody EditAvatarDto dto) {
        return ResponseEntity.ok(userDetailsMapper.toResponse(userDetailsService.editAvatar(userId, dto)));
    }
