...
- rate limiting через Redis на gateway;
- Redis-кеш проверки membership в `message-service`;
...
- CRUD-операции по пользователям и поиск по тегу.
- Создание/получение/удаление чатов.
- Создание/чтение/редактирование/удаление сообщений.
...
| ------------------------ | -----: | ------------------- | ---------------------------------------------------- |
| `gateway-service`        | `8080` | Redis (`6380`)      | Маршрутизация, проверка токена, rate limiting, WebSocket-мост |
| `authentication-service` | `8081` | PostgreSQL (`5433`) | Регистрация, логин, JWT, валидация токена            |
| `user-service`           | `8082` | PostgreSQL (`5432`) | Профиль пользователя, теги, поиск                     |
| `chat-service`           | `8083` | PostgreSQL (`5434`) | Чаты, участники, обновление `lastMessageAt`          |
| `message-service`        | `8084` | MongoDB (`27017`) + Redis (`6379`) | Сообщения, read/edit/delete, Redis-кеш membership |
| `common-libs`            |      - | -                   | Общие DTO, мапперы, константы                        |

## Как проходят запросы

1. Клиент отправляет запрос на `gateway-service`.
2. Для всех путей, кроме `/auth/**`, gateway вызывает `/auth/validate` и применяет rate limiting.
...
3. `message-service` публикует событие в Kafka (`chat-messages`).
4. `chat-service` обновляет `lastMessageAt` у чата.
5. `gateway-service` читает событие и отправляет его онлайн-участникам чата по WebSocket.
...
3. Изменения уходят в Kafka и доставляются участникам чата через gateway.
...
1. `GET /user/search?tag=...` ищет пользователей по тегу.
2. Результат ограничен 10 совпадениями.
...
- для Redis: `REDIS_PASSWORD` (gateway и message-service).
...
- Redis для gateway: `localhost:6380`
- Redis для message-service: `localhost:6379`
